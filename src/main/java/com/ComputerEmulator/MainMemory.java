...
public class MainMemory {                                                       
    ...
    public static void load(String[] data) throws Exception {      
        ...
        for (String str : data) {
            ...
            for (int x = 0; x < 32; x++) {
                switch (str.charAt(x)) {
                    case '0' -> mainMem[index].setBit(x, new Bit(false));
                    case '1' -> mainMem[index].setBit(x, new Bit(true));
                    default -> throw new Exception("Input String number " + ++index + " must only contain 0's and 1's.");
                }
            }
            index++;
        }
    ...
    public static void load(String fileName) throws Exception {     
        load(Files.readAllLines(Paths.get(fileName)).toArray(String[]::new));                                                        
    }
}
